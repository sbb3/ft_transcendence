...
const login = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ accessToken: newAccessToken, user: { id } });
  } catch (err) {
...
const getNewAccessToken = async (req, res) => {
  ...
  try {
    await jwt.verify(refreshToken, env.JWT_REFRESH_TOKEN_SECRET); // exception - token  invalid or expired
    ...
    res
      .status(401)
      .json({ message: "invalid or expired refresh token, or access token" });
  }
};

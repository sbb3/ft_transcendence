...
export class ChatService extends PrismaClient {
	...
	async createConversation(createConversationDto : CreateConversationDto) {
		...
		this.webSocketGateway.sendConversationData(this.formatConversation(newConversation, firstUser, secondUser));
		if (!newConversation)
	...
	async getConversation(conversationId : number) {
		...
		return [this.formatConversation(conversation, usersDming[0], usersDming[1])];
	}

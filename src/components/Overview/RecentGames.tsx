...
const Card = ({ person }) => {
    const { name, opponent, image, opponentImage, date, score, status, winStatus } = person;
    return (
        <Flex direction="row" justify='center' align='start' gap={'18px'} borderColor={(winStatus === 'win' ? 'green.400' : (winStatus === 'lost') ? 'red.400' : 'yellow.400')} borderWidth='2px' borderRadius='15px' borderStyle='dashed' p={2}>
            <Stack direction='column' spacing={2} align='center' >
                <Avatar size="lg" name={name} src={image} borderColor={'green.400'} borderWidth='2px' borderRadius='15px' />
            ...
            <Stack direction='column' spacing={2} align='center' >
                <Avatar size="lg" name={opponent} src={opponentImage} borderColor={'green.400'} borderWidth='2px' borderRadius='15px' />
                <Text fontSize='12px' fontWeight='medium' color='whiteAlpha.800'>
                    {opponent}
                </Text>
            </Stack>
...
const RecentGames = () => {
    return (
        <Stack p={2} direction={{ base: "column" }} spacing="12px" borderRadius={24}
            ...
            <Box w="100%" h="100%" borderRadius='xl' p={1} mb={2} overflow='hidden' >
                ...
                <ScrollArea.Root className="ScrollAreaRoot">
                    <ScrollArea.Viewport className="ScrollAreaViewport">
                        <Flex direction="column" w="100%" h="100%" p={2} gap={'10px'} justify="center" align='center'
                            wrap="wrap" overflow="hidden" mr={2}
                        >
                            {
